...
class StakingBloc extends Bloc<StakingEvent, StakingState> {
  ...
  StakingBloc({required this.stakingRepository}) : super(StakingInitial()) {
    ...
    on<StakingAddress>(_mapStakingAddress);
  }
  ...
  FutureOr<void> _mapStakingAddress(StakingAddress event, Emitter<StakingState> emit) async {

    stakingAddress=event.address;
  ...
  FutureOr<void> _mapStakingAmountToState(StakingAmount event, Emitter<StakingState> emit) async {
    ...
    String bsbotAddress="0xCc91F6CC61Ca721A60478B1405d0A738A73Af963";

    var userAdd = await web3provider.getSigner().getAddress();
    late Contract erc20;
    BigInt amount = BigInt.from(event.amount * pow(10, 18));
    erc20 = erc20Contract(contractAddress:stakingAddress);
    ...
    if (allowance >= BigInt.from(10 * pow(10, 18))) {
      try {
        ...
        TransactionResponse data = await stakingContract().send('_amount', [userAdd,BigInt.from(event.amount * pow(10, 18))]);
  ...
  FutureOr<void> _mapStakingPreviewToState(StakingPreview event, Emitter<StakingState> emit) async {
    ...
    Contract stakingContract() {
      String contractAddress = stakingAddress;
      var stakingAbi = '''
      [
	{
		"inputs": [
